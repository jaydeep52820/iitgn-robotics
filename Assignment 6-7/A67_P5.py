...
dist1 = np.random.normal(loc=0.0, scale=0.1, size=1000)
dist2 = np.random.normal(loc=0.0, scale=0.1, size=1000)
...
def modelD(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2):
    theta1 = y[0]
    theta1d = y[1]
    theta2 = y[2]
    theta2d = y[3]
    ...
    disturb1 = disturbance(t, dist1)
    disturb2 = disturbance(t, dist2)
    ...
    U = np.matmul(MM, aq) + np.matmul(C, np.transpose([theta1d, theta2d, dd])) + G + np.transpose([disturb1, disturb2, 0])
...
def modelC(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2):
    theta1 = y[0]
    theta1d = y[1]
    theta2 = y[2]
    theta2d = y[3]
    ...
    u1 = 2 * e1 + 1 * e1_d + (alpha + beta + 2 * gamma * np.cos(d_theta2) + 0.01) * d_theta1dd + np.matmul(MM_Des[0], np.transpose([d_theta1dd, d_theta2dd, 0])) + \
         np.matmul(C_des[0], np.transpose([d_theta1d, d_theta2d, 0])) + disturb1
    u2 = 2 * e2 + 1 * e2_d + (beta + 0.01)*d_theta2dd + np.matmul(MM_Des[1], np.transpose([d_theta1dd, d_theta2dd, 0])) + \
         np.matmul(C_des[1], np.transpose([d_theta1d, d_theta2d, 0])) + disturb2
...
# function that returns dy/dt
def modelB(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, disrt2):
    theta1 = y[0]
    theta1d = y[1]
    theta2 = y[2]
    theta2d = y[3]
    ...
    u1 = 2 * e1 + 1 * e1_d + (alpha + beta + 2*gamma*np.cos(theta2) + 0.01)*d_theta1dd + disturb1
    u2 = 2 * e2 + 1 * e2_d + + (beta + 0.01)*d_theta2dd + disturb2
...
# function that returns dy/dt
def modelA(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2):
    theta1 = y[0]
    theta1d = y[1]
    theta2 = y[2]
    theta2d = y[3]
    ...
    u1 = 2 * e1 + 1 * e1_d + disturb1
    u2 = 2 * e2 + 1 * e2_d + disturb2
...
# solve ODE
yA = odeint(modelA, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2))
yB = odeint(modelB, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2))
yC = odeint(modelC, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2))
yD = odeint(modelD, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb, dist1, dist2))
...
fig1.suptitle('Problem5: End Effector motion from A SCARA Robot using VARIOUS Controller, with disturbances, mean = 0 and SD = 0.1')
...
plt.xlabel('t (sec)')
plt.ylabel('x (m)')
fig2.suptitle('Problem5: End Effector motion from A SCARA Robot using VARIOUS Controller, with disturbances, mean = 0 and SD = 0.1')
...
plt.xlabel('x (m)')
plt.ylabel('y (m)')
fig3.suptitle('Problem5: End Effector motion from A SCARA Robot using VARIOUS Controller, with disturbances, mean = 0 and SD = 0.1')
...
fig4.suptitle('Problem5: End Effector motion from A SCARA Robot using VARIOUS Controller, with disturbances, mean = 0 and SD = 0.1')
...
fig5.suptitle('Problem5: End Effector motion from A SCARA Robot using VARIOUS Controller, with disturbances, mean = 0 and  SD = 0.1')
...
fig5 = plt.figure()
plt.plot(t, dA, label='theta1')
plt.plot(t, dB, label='theta1')
plt.plot(t, dC, label='theta1')
plt.plot(t, dD, label='theta1')
plt.plot(t, d_d, color='y', label='theta1', alpha=0.3, linewidth=4)
plt.legend(['dA', 'dB', 'dC', 'dD', 'desired-d'])
plt.xlabel('t (sec)')
plt.ylabel('d (m)')
