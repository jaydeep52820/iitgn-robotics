...
# New parameters with 20% error

# For Arm1
l1_e = 0.3*0.8   # m, Length of arm1
r1_e = l1/2
m1_e = (math.pi * R1 * R1 * l1_e) * D1      # kg, Mass of Arm 1
J1_e = (1 / 3) * m1 * l1_e * l1_e     # Inertia about end point for Arm1

# For Arm2
l2_e = 0.2*0.8   # m,Length of Arm2
r2_e = l2/2
m2_e = (math.pi * R2 * R2 * l2_e) * D2     # kg, Mass of Arm 2
J2_e = (1 / 3) * m2 * l2_e * l2_e             # Inertia about end point for Arm2]

# For Arm3
l3_e = 0.1      # m,Length of Arm2
r3_e = l3/2
m3_e = (math.pi * R3 * R3 * l3_e) * D3     # kg, Mass of Arm 2
J3_e = (1 / 3) * m3 * l3_e * l3_e            # Inertia about end point for Arm2
...
def modelD(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb):
    ...
    alpha_e = J1_e + r1_e * r1_e * m1_e + l1_e * l1_e * m2_e + l1_e * l1_e * m3_e
    beta_e = J2_e + J3_e + l2_e * l2_e * m3_e + m2_e * r2_e * r2_e
    gamma_e = l1_e * l2_e * m3_e + l1_e * r2_e * m2_e
    ...
    # Dynamics for controller (with error)
    MM_e = np.array([[alpha_e + beta_e + 2 * gamma_e * np.cos(theta2), beta_e + 2 * gamma_e * np.cos(theta2), 0],
                   [beta_e + 2 * gamma_e * np.cos(theta2), beta_e, 0],
                   [0, 0, m3_e]]) + J
    C_e = np.array([[-gamma_e * np.sin(theta2) * theta2d, -gamma_e * np.sin(theta2) * (theta2d + theta1d), 0],
                  [gamma_e * np.sin(theta2) * theta1d, 0, 0],
                  [0, 0, 0]]) + B
    G_e = np.transpose(np.array([0, 0, m3_e * g]))
    ...
    U = np.matmul(MM_e, aq) + np.matmul(C_e, np.transpose([theta1d, theta2d, dd])) + G_e
...
def modelC(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb):
    ...
    MM_Des = np.array([[0, beta_e + 2*gamma_e*np.cos(d_theta2), 0],
                   [beta_e + 2*gamma_e*np.cos(d_theta2), 0, 0],
                   [0, 0, 0]])

    C_des = np.array([[-gamma_e * np.sin(d_theta2) * d_theta2d, -gamma_e * np.sin(d_theta2) * (d_theta2d + d_theta1d), 0],
                  [gamma_e * np.sin(d_theta2) * d_theta1d, 0, 0],
                  [0, 0, 0]]) + B

    G_des = np.transpose(np.array([0, 0, m3_e * g]))

    # Dynamics equations from: Dynamic modeling of SCARA robot based on Udwadia–Kalaba theory by Yaru Xu and Rong Liu

    u1 = 2 * e1 + 1 * e1_d + (alpha_e + beta_e + 2*gamma_e*np.cos(d_theta2) + 0.01)*d_theta1dd + np.matmul(MM_Des[0], np.transpose([d_theta1dd, d_theta2dd, 0])) + \
         np.matmul(C_des[0], np.transpose([d_theta1d, d_theta2d, 0]))
    u2 = 2 * e2 + 1 * e2_d + (beta_e + 0.01)*d_theta2dd + np.matmul(MM_Des[1], np.transpose([d_theta1dd, d_theta2dd, 0])) + \
         np.matmul(C_des[1], np.transpose([d_theta1d, d_theta2d, 0]))
...
def modelB(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb):
    ...
    u1 = 2 * e1 + 1 * e1_d + (alpha_e + beta_e + 2*gamma_e*np.cos(d_theta2) + 0.01)*d_theta1dd
    u2 = 2 * e2 + 1 * e2_d + + (beta_e + 0.01)*d_theta2dd
    u3 = 50*e3 + 0.001*e3_d + m3*g + m3_e*0
...
def modelA(y, t, m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb):
    ...
    # Controller does not depend on length of links
...
# solve ODE
yA = odeint(modelA, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb))
yB = odeint(modelB, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb))
yC = odeint(modelC, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb))
yD = odeint(modelD, y0, t, args=(m1, m2, m3, l1, l2, l3, r1, r2, r3, J1, J2, J3, g, yda, ydb))
...
fig.suptitle('Problem4: End Effector motion from A SCARA Robot using VARIOUS Controller, with 20% error in link 1 and 2')
